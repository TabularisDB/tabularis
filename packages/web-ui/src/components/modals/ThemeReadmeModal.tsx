...
export function ThemeReadmeModal({ plugin, registryUrl, onClose }: Props) {
  ...
  useEffect(() => {
    let disposed = false;
    void client.call("fetch_plugin_readme", { slug: plugin.id, locale, registryUrl }).then((data) => {
  ...
  }, [client, plugin.id, registryUrl, locale]);
  const open = (url: string) => { void platform.openExternalUrl(url).catch((failure) => setError(String(failure))); };
