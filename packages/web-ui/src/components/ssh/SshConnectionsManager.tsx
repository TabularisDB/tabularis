...
interface SshConnectionsManagerProps {
  /**
   * Overlay class forwarded to the delete ConfirmModal, e.g. to stack it
   * above a parent modal. Defaults to the standard Modal overlay.
   */
  confirmOverlayClassName?: string;
}

/**
 * Self-contained list + editor for saved SSH connections. Rendered both by
 * SshConnectionsModal (connection-creation flow) and by the Settings SSH tab,
 * so all state and handlers live here — not in the hosts.
 */
export function SshConnectionsManager({
  ...
  const requestDelete = async (id: string) => {
    ...
    try {
      ...
      // Best-effort warning only — deletion is still confirmed by the user.
      console.error("Failed to count connections using SSH tunnel:", error);
    }
  };
  ...
  return (
    <>
        ...
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t border-strong">
            <button
              ...
              className="w-full px-4 py-2 border border-strong rounded-lg hover:bg-elevated transition-colors text-secondary text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {testStatus === "testing" && (
                <Loader2 size={16} className="animate-spin" />
              )}
          ...
          <div className="flex gap-3 pt-4">
            <button
              onClick={resetForm}
              className="flex-1 px-4 py-2 border border-strong rounded-lg hover:bg-elevated transition-colors text-secondary text-sm font-medium"
            >
      ...
      <ConfirmModal
        ...
        message={
          deleteUsageCount > 0
            ? `${t("sshConnections.confirmDelete")} ${t("sshConnections.deleteInUse", { count: deleteUsageCount })}`
            : t("sshConnections.confirmDelete")
        }
