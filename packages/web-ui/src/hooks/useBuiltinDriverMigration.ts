...
import type { ConnectionParameters } from "../api/contract";

/** Which surface the banner should show. Driven by the connectivity gate. */
/** The saved connection's parameters with only the driver swapped. */
function paramsWithDriver(
  conn: SavedConnection | undefined,
  driver: string,
): ConnectionParameters {
  return { database: "", ...conn?.params, driver };
}
...
export function useBuiltinDriverMigration(
      ...
      const run = async (): Promise<MigrationOutcome> => {
        try {
          ...
          await client.call("update_connection", {
            ...
            params: paramsWithDriver(conn, pluginId),
            detectJsonInTextColumns: conn?.detect_json_in_text_columns ? true : undefined,
        ...
        try {
          registeredDrivers = await client.call("get_registered_drivers", undefined);
        } catch (e) {
          const error = e instanceof Error ? e.message : String(e);
          const outcome: MigrationOutcome = {
            connectionId,
        ...
        if (!pluginRegistered) {
          ...
          try {
            const startupErrors = await client.call(
        ...
        try {
          ...
          try {
            await client.call("test_connection", {
              request: {
                params: paramsWithDriver(conn, builtinId),
                connection_id: connectionId,
              },
            });
          } catch {
  ...
  const undoMigration = useCallback(
    ...
    [client, builtinId, pluginId, connect, disconnect, loadConnections],
  );
