...
/**
 * Owns the state and backend call for running EXPLAIN / EXPLAIN ANALYZE. Shared by
  ...
  const runExplain = useCallback(
    async ({ connectionId, query, analyze = false, schema = null }: RunExplainArgs) => {
      ...
      await loadPlan(async () => {
        ...
        const result = await client.call("explain_query_plan", {
          connectionId,
          query,
          analyze,
    ...
    [client, invalidate, loadPlan, reset, t],
  );
