...
vi.mock("@tauri-apps/plugin-fs", () => ({ writeTextFile: vi.fn(), readFile: vi.fn(), writeFile: vi.fn() }));

const mockSave = vi.mocked(save);
// Exports go through the platform download, which writes the encoded bytes.
...
describe("ResultToolbar", () => {
  describe.each(formats)("$label export", ({ format, label, output }) => {
    ...
    it("saves the exact serialized output and shows one success alert", async () => {
      ...
      expect(mockWriteFile).toHaveBeenCalledExactlyOnceWith(filePath, new TextEncoder().encode(output));
    ...
    it("waits for the write to resolve before showing success", async () => {
      ...
      mockWriteFile.mockReturnValueOnce(pendingWrite);
