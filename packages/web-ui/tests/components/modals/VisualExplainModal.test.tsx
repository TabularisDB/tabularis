...
describe("VisualExplainModal legacy execution", () => {
  ...
  it("runs SELECT with Analyze enabled and null schema exactly once under StrictMode", async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledExactlyOnceWith("explain_query_plan", {
      connectionId: "connection-1", query: defaults.query, analyze: true, schema: undefined,
    });
  ...
  it.each([
    ...
    expect(mockInvoke).toHaveBeenLastCalledWith("explain_query_plan", {
      connectionId: "connection-1", query: defaults.query, analyze: true, schema: undefined, ...change,
    });
  ...
  it.each([
    ...
    expect(screen.getByText("editor.visualExplain.analyzeWarning")).toBeInTheDocument();
    expect(mockInvoke).toHaveBeenLastCalledWith("explain_query_plan", {
      connectionId: "connection-1", query, analyze: false, schema: undefined,
    });
    await act(async () => {
  ...
  it("runs a source change with the Analyze choice recorded for that source", async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledTimes(2);
    expect(mockInvoke).toHaveBeenLastCalledWith("explain_query_plan", {
      connectionId: "connection-1", query: defaults.query, analyze: false, schema: undefined,
    });
  });
