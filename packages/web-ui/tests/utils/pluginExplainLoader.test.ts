...
const readAsset = (pluginId: string, filePath: string) =>
  invokeMock("read_plugin_file", { pluginId, filePath }) as Promise<string>;
...
describe("pluginExplainLoader", () => {
  it("registers a bundle parser that parseRawExplain can dispatch to", async () => {
    ...
    const formats = await loadPluginExplainParsers(pluginManifest, readAsset);
  ...
  it("stops before evaluating a bundle once loading is cancelled", async () => {
    ...
    const formats = await loadPluginExplainParsers(pluginManifest, readAsset, () => !cancelled);
  ...
  it("can load again after the previous pass unregisters its formats", async () => {
    ...
    const firstPass = await loadPluginExplainParsers(pluginManifest, readAsset);
    firstPass.forEach(unregisterExplainParser);
    const secondPass = await loadPluginExplainParsers(pluginManifest, readAsset);
