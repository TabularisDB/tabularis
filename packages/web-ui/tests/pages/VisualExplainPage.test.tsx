...
describe("VisualExplainPage", () => {
  beforeEach(() => {
    ...
    // Picked files are read through the platform adapter (plugin-fs) rather
    // than `load_explain_from_file`. Route those reads through the same invoke
    // stub so each scenario keeps describing one backend read per picked file.
    vi.mocked(readFile).mockReset();
    vi.mocked(readFile).mockImplementation(async (path) => {
      const file = await invoke<{ content: string }>("load_explain_from_file", { path });
  ...
  it("prefers a deep link over a file and preserves safe invoke options", async () => {
    ...
    expect(invoke).toHaveBeenCalledExactlyOnceWith("explain_query_plan", {
      connectionId: "connection-1", query: "SELECT * FROM users", analyze: false, schema: undefined,
    });
  });
  ...
  it("opens a selected file with the existing picker options and can reload it", async () => {
    ...
    expect(readFile).toHaveBeenLastCalledWith("/tmp/picked.json");
    ...
    // The platform exposes a picked file by name; its path stays an opaque reference.
    expect(screen.getByTitle("picked.json")).toHaveTextContent("picked.json");
  });
  ...
  it.each(["resolve", "reject"] as const)("ignores a stale CLI lookup %s after a file is selected", async (outcome) => {
    ...
    expect(invoke).toHaveBeenCalledTimes(2);
    expect(screen.getByTitle("chosen.json")).toBeInTheDocument();
