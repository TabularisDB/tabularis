...
// Mock the four context hooks the migration hook composes, plus the client.
...
// The hook talks to the backend through the Tabularis client; forward its
// calls to the `invoke` mock so per-command assertions stay transport-agnostic.
vi.mock("../../src/hooks/useTabularisClient", async () => {
  const { invoke } = await import("@tauri-apps/api/core");
  const client = {
    call: (command: string, request: unknown) =>
      invoke(command, request as Record<string, unknown> | undefined),
  };
