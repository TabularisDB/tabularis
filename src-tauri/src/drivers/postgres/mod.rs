...
fn primary_key_exists_expression() -> &'static str {
    r#"
            EXISTS (
                SELECT 1
...
pub async fn get_columns(
    ...
    "#,
        primary_key_exists_expression(),
    );

    let rows = query_all(&pool, query.as_str(), &[&schema, &table_name]).await?;
...
pub async fn get_all_columns_batch(
    ...
    let rows = query_all(&pool, query.as_str(), &[&schema]).await?;
...
pub async fn get_view_columns(
    ...
    let rows = query_all(&pool, query.as_str(), &[&schema, &view_name]).await?;
