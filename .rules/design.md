...
6. **Libraries that need real colors** (ReactFlow, Recharts) read `currentTheme.colors` via `useTheme()`. Inline styles needing alpha use `tint()` from `packages/web-ui/src/utils/tones.ts` or `var(--token)`.
7. **Contrast is part of the contract.** Built-in themes must meet WCAG AA on the pairs in `CONTRAST_PAIRS` (`packages/web-ui/src/utils/themeContrast.ts`): fix a failing pair by moving lightness, keep the hue. A new text/background combination goes into `CONTRAST_PAIRS` first. See "Contrast" in DESIGN.md.
